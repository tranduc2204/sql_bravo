...
--WHERE EffectiveDate = '2015-01-01' AND ItemCode NOT IN (SELECT ItemCode FROM #B20Prices WHERE EffectiveDate < '2015-01-01');
	...
	--h.	Cập nhật giá bán của các đơn hàng theo bảng giá
	--select * 
	--from #B30AccDocDetail;
	

	--- giá của từng sản phẩm
	select code, Name_, EffectiveDate, Amount
	into #tmp1
	from #B20Item tb1 
	join #B20Prices tb2
		on tb1.Code = tb2.ItemCode
	...
	select id, DocNo, DocDate, CustomerCode, ItemCode, Quantity, tb2.Amount, Quantity * tb2.Amount
	from #B30AccDocDetail tb1
	join #tmp1 tb2
