...
BEGIN 
	...
	SELECT @PivotColumns = COALESCE (@PivotColumns + ',','') + QUOTENAME (WarehouseCode)
	FROM (SELECT DISTINCT WarehouseCode FROM #B30OpenStock) AS Warehouse;
	...
	PIVOT(
		SUM(Quantity)
		FOR WarehouseCode IN ('+@PivotColumns +')
	) AS PivotTable 
	
	
	SELECT '''' AS GroupCode, '''' as ItemCode, N''Tong cong'' as Name, '+@PivotColumns+'
	INTO #TMP2
	FROM (
	...
	SELECT GroupCode, ItemCode, Name AS ItemName, '+@PivotColumns+'
	FROM #TMP1
	UNION ALL
	SELECT GroupCode, ItemCode, Name AS ItemName, '+@PivotColumns+'
	FROM #TMP2'
