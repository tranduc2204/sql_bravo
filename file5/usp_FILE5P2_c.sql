...
--c.	Tìm nhân viên không bán được hàng trong trong tháng kinh doanh cuối cùng

DROP PROC IF EXISTS usp_FILE5P2_c
GO 
CREATE PROC usp_FILE5P2_c
AS 
BEGIN
	...
	--c.	Tìm nhân viên không bán được hàng trong trong tháng kinh doanh cuối cùng
	select * from #B20Employee
	where #B20Employee.Code not in (select distinct #B30AccDocSales.EmployeeCode 
									from #B30AccDocSales
									where month (DocDate) = 12 and year(DocDate) = 2014 )
	;


	select distinct #B30AccDocSales.EmployeeCode 
									from #B30AccDocSales
									where month (DocDate) = 12 and year(DocDate) = 2014 ;
