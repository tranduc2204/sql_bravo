...
--l.	Tìm mặt hàng bán số lượng nhiều nhất trong từng năm

DROP PROC IF EXISTS usp_FILE5P2_l
GO 
CREATE PROC usp_FILE5P2_l
AS 
BEGIN
...
--l.	Tìm mặt hàng bán số lượng nhiều nhất trong từng năm 
	...
	select TB2.Code,  max (TB2.Name) Name, sum (Quantity) Quantity , year (DocDate) nam
	into #tmp
	...
	select  *, ROW_NUMBER () over (partition by nam order by Quantity desc) as rank
	into #tmp1
	from #tmp
	;
