...
--d.	Tìm nhân viên bán hàng có doanh số bán nhiều nhất trong từng tháng của năm 2014

DROP PROC IF EXISTS usp_FILE5P2_d
GO 
CREATE PROC usp_FILE5P2_d
AS 
BEGIN
	...
	VALUES
		...
		(3, '2023-01-03', 'IT003', 'C003', 'E002', 'D003', 4, 800.00),
...
--d.	Tìm nhân viên bán hàng có doanh số bán nhiều nhất trong từng tháng của năm 2014
	select  EmployeeCode, max (tb2.Name) Name , MONTH (DocDate) thang, YEAR (DocDate) nam, sum (Quantity * Amount) doanhso,
	ROW_NUMBER() OVER (PARTITION BY MONTH(DocDate), YEAR(DocDate) ORDER BY SUM(Quantity * Amount) DESC) AS Rank_
	into #tmp
	from #B30AccDocSales tb1 
	join #B20Employee tb2 
		on tb1.EmployeeCode = tb2.Code
	group by EmployeeCode, MONTH (DocDate), YEAR (DocDate)
	;
	select * from #tmp
	where Rank_ = 1;
