...
--e.	Tìm 3 nhân viên bán hàng có doanh số bán nhiều nhất trong năm 2014


DROP PROC IF EXISTS usp_FILE5P2_e
GO 
CREATE PROC usp_FILE5P2_e
AS 
BEGIN
	...
	VALUES
		...
		(3, '2014-01-03', 'IT003', 'C003', 'E002', 'D003', 4, 800.00),
		...
		(7, '2014-01-07', 'IT007', 'C007', 'E007', 'D007', 4, 800.00),
...
--e.	Tìm 3 nhân viên bán hàng có doanh số bán nhiều nhất trong năm 2014
	select top 3 with ties tb1.Code , max (tb1.Name), sum(Quantity * Amount) doanhso
	from #B20Employee tb1
	join #B30AccDocSales tb2 
		on tb1.Code = tb2.EmployeeCode
