...
--h.	Tìm khách hàng chỉ mua hàng trong tháng 7 năm 2014


DROP PROC IF EXISTS usp_FILE5P2_h
GO 
CREATE PROC usp_FILE5P2_h
AS 
BEGIN
	...
	VALUES
		...
		(3, '2014-07-03', 'IT003', 'C003', 'E002', 'D003', 4, 800.00), --- e2 mua tháng 7 mà ucngx mua ở tháng 12 nên cũng cúcc
...
--h.	Tìm khách hàng chỉ mua hàng trong tháng 7 năm 2014
	...
	WHERE Code not IN (
					...
					from #B30AccDocSales
					where DocDate not between '2014-7-1' and '2014-7-31');


	--SELECT * 
	--FROM #B20Customer  CUS
	--WHERE EXISTS (
	--				select distinct CustomerCode 
	--				from #B30AccDocSales
	--				where month (DocDate) =7 AND YEAR (DocDate) = 2014 AND CUS.Code = CustomerCode)
