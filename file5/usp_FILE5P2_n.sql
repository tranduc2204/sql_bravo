...
--n.	Tìm mặt hàng bán nhiều nhất về số lượng nhưng không nhiều nhất về doanh thu trong từng năm


DROP PROC IF EXISTS usp_FILE5P2_n
GO 
CREATE PROC usp_FILE5P2_n
AS 
BEGIN
...
--n.	Tìm mặt hàng bán nhiều nhất về số lượng nhưng không nhiều nhất về doanh thu trong từng năm
	...
	SELECT TB2.Code, MAX(TB2.Name) AS Name, SUM(Quantity) AS Quantity, YEAR(DocDate) AS Nam
	FROM #B30AccDocSales TB1
	JOIN #B20Item TB2 ON TB1.ItemCode = TB2.Code
	GROUP BY TB2.Code, YEAR(DocDate);

	SELECT TB2.Code, MAX(TB2.Name) AS Name, SUM(Quantity * Amount) AS DoanhThu, YEAR(DocDate) AS Nam
		FROM #B30AccDocSales TB1
		JOIN #B20Item TB2 ON TB1.ItemCode = TB2.Code
		GROUP BY TB2.Code, YEAR(DocDate);
	...
	SELECT TB1.CODE, TB1.NAME, Quantity, DoanhThu, TB1.NAM
	FROM QuantityMax TB1
	JOIN RevenueMax TB2
		ON TB1.Code = TB2.Code
	WHERE TB1.Quantity >= (SELECT MAX (Quantity)
							FROM QuantityMax
							WHERE NAM = TB1.Nam)
	AND TB2.DoanhThu < (SELECT MAX (DoanhThu)
						FROM RevenueMax
						WHERE NAM = TB2.Nam);
